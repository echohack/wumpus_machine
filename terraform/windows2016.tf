...
cmd.exe /c net user wumpus ${random_string.random.result} /add /LOGONPASSWORDCHG:NO
cmd.exe /c net localgroup Administrators /add wumpus
...
cmd.exe /c wmic useraccount where "name='wumpus'" set PasswordExpires=FALSE
...
set-executionpolicy -executionpolicy unrestricted -force -scope LocalMachine
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "powershell.exe C:/ProgramData/chocolatey/choco install discord -y",
      "powershell.exe C:/Users/Administrator/AppData/Local/Discord/Update.exe --processStart Discord.exe"
    ]
  }
}
